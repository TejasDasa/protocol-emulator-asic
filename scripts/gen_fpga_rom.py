...
def main():
    ...
    # The exact command that reproduces this file, rebuilt from the parsed
    # arguments rather than from sys.argv, so it is canonical and complete
    # whatever mix of flags and defaults was typed. scripts/check_rom_fresh.py
    # re-runs it and diffs, which is what stops a tracked ROM drifting behind
    # the generator -- as it did once, when stt_fpga_top gained references to
    # localparams the committed stt_rom.vh did not yet define.
    defaults = {act.dest: act.default for act in ap._actions}
    repro = ["python3", "scripts/gen_fpga_rom.py"]
    for dest in ("program", "nsm", "div", "fclk", "baud", "byte", "sck_div",
                 "jedec_mid", "usb_period", "period", "seconds"):
        val = getattr(a, dest, None)
        if val is None or val == defaults.get(dest):
            continue
        flag = "--" + dest.replace("_", "-")
        if dest in ("byte", "jedec_mid"):
            repro += [flag, hex(val)]
        else:
            repro += [flag, f"{val:g}" if isinstance(val, float) else str(val)]
    repro += ["-o", a.out or "rtl2/stt_rom.vh"]
    repro_line = " ".join(repro)
