...
| The **tracked** `rtl2/stt_rom.vh` is byte-identical to what the generator produces now, so a Vivado build (which never runs the generator) sees the same localparams the testbenches do | `make check-rom-fresh` (`scripts/check_rom_fresh.py`): re-runs the `Reproduce with:` command from the file's own header and diffs. Verified in both directions -- deleting one localparam from the tracked file makes it fail | 2026-09-26 |
