...
| **Pair symmetry on D+/D− physically** | `rtl2/tb/tb_fpga_usb.v` asserts D+ and D− are never equal except during deliberate SE0 | On the board the LED counts *bytes*. A pair-slot fault driving only one half would still produce decodable single-ended traffic and the LED would still blink. Only the simulation checks symmetry |
