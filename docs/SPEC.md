...
| **Slew, capacitance and fanout** | Not clean, post-route on the five-machine D design at the slow corner: **26 max-slew** (3 typ, 0 fast), **7 max-capacitance**, **621 max-fanout**. Routing and setup/hold timing are clean; these are not. Of the fanout violations **320 are inside the CFGMEM macros** — the macro's own `SLICE[*].ROW_AND` and `SELBUF` drive 33 loads against a limit of 8 — and are a property of a vendored macro, not of this design or fixable by resizing it. The remaining 301 fanout, and all the slew and capacitance violations, are in our logic. Traced, not guessed: **4 of the 7 capacitance violations and the worst slew violations are on `u_imem.do_tile`**, the CFGMEM macro's `Do0` output driving the asynchronous cross-tile read mux. SPEC §12 requires that read to be asynchronous, so it cannot be registered away; the macro is a latch array with weak output drive, and the load is 32 bits of mux plus long wires across macro shadows. That load is only known after routing -- and `RUN_POST_GRT_DESIGN_REPAIR` and `RUN_POST_GRT_RESIZER_TIMING` are both **off**, so the one repair pass that could buffer it never runs. Enabling them is the targeted fix and has not yet been tried. |
...
| **`PSM-0069` blocks IR-drop analysis** | `OpenROAD.IRDropReport` fails `Check connectivity failed on VPWR` on both C and D at ten macros, and passes at two (`pdn_test`, which reports `PSM-0040 All shapes on net VPWR are connected`). **The mechanism is NOT understood.** An earlier entry here asserted it was the `ExtendPowerStripes` plugin failing to write the ODB `iterm`-to-net annotation; that was tested and is wrong. Adding the annotation (10 VPWR and 10 VGND macro pins connected in the database, confirmed in the log) left `PSM-0069` firing and the count of `PSM-0038` unconnected shapes unchanged at 25, while adding 11 new `PSM-0039` unconnected-instance warnings. The evidence against the macro-pin theory is positional: the 25 unconnected `VPWR` shapes run x = 55.82 to 1089.90 at the 44.96 um stripe pitch, i.e. **every vertical Metal4 stripe across the whole core**, while the macro columns are only at x = 45.43, 405.11 and 764.79. So the whole stripe network lacks a source in PSM's view, at ten macros but not at two, and why is open. Signoff DRC and LVS are still reachable by disabling the IR-drop step, which is how those numbers were obtained. |
