...
**SPECIFIED — the free-running rule holds under every sequence of timer
actions, and it is formally proved.** The counter never exceeds `P−1`. Between
two consecutive ticks with no `trst` or `thalf` in between, exactly `P` cycles
elapse, so `tmr` is true on exactly one cycle in `P` — for every `P`, and
whether the intervening rows passed their tests or failed them. A `trst` or
`thalf` on a row whose test **failed** changes nothing. Proved unbounded in
`docs/formal.md` P5, subject to `P ≥ 2` for the reason recorded in §16.1.
...
| **USB low speed, end to end** | **CLOSED on hardware**, both routes. `usb_ls_token_tx` on machine 0 and `stt_usb_rx` on machine 1, on a Cora Z7-07S at STT_NSM=2, with D+ leaving the chip on `uo_out[0]` and arriving on `uio[0]` -- internally routed, and again through a jumper across real pads. This is the first physical exercise of the **pair slot** (§7 code 3, one row writing two pins), of **NRZI** in both directions, of the **self-loopback through a `uio` pin** that NRZI decoding needs (§16.0), and of the §11.2 host **write** path, which `usb_ls_token_tx` forces because it takes three different bytes through `load` and `loadk` supplies one constant. Simulated first in `rtl2/tb/tb_fpga_usb.v`, which checks what a waveform cannot: that the pair moves TOGETHER, since a pair-slot fault driving one half still produces plausible single-ended traffic. **The ASIC has not been fabricated** -- this is FPGA hardware, and what it establishes about the ASIC is that the mechanism needs a `uio` pin and not a board wire. |
| **USB receive needs an idle gap, and it is measured** | The receiver ends a packet by seeing a run of ones longer than stuffing permits, because it cannot see SE0: it has two inputs and both are spent, one on D+ and one on the decoded-bit loopback. The transmitter sends SE0, SE0, J and then starts the next packet at once, so back to back there is **one** idle bit, the run never builds, and consecutive packets run together. **Measured twice, on different plumbing**: in the model, and again on the FPGA where the host driver was keeping the TX FIFO fed. Both times the signature was the same and is worth recognising -- **packet 1 byte-exact, everything after it garbage**. A host must leave about 12 bit times; `STT_ROM_HOST_WGAP` carries `16 x P`. This is a requirement of THIS receiver, not of USB (§16.0). |
...
| **Instruction-memory load handshake in testbenches** | CLOSED, with a directed test. The loader raises `imem_ld_busy` on `uo_out[0]` and the host must not present a bit while it is high (SPEC section 10). A testbench that samples `uo_out` in the same delta as the clock edge reads the flag as it was BEFORE the edge, so a busy that has just risen reads as idle and exactly one bit is lost at every 32-bit word boundary: word N lands as word N shifted right by N. Row 0 survives, so the machine starts and then executes a zeroed row -- it looks alive and drives nothing. `rtl2/tb/test_imemload.py` (`make rtl2-imemload`) reads the CFGMEM contents back and compares word for word for five programs, and names the mechanism when it fails. Removing the settle again makes it fail. |
| **Input-side setup and hold** | NOT MEASURED. Everything in `docs/skew-report.md` is output-to-output. Whether a pin a machine *reads* -- MISO, TDO, SDA when the target drives it -- meets setup at the first flop of the §8.3 synchronizer under real cell and routing delay is a separate question that the output measurement says nothing about. The margins are expected to be similarly large, since the synchronizer already imposes a two-cycle reaction delay, but expected is not measured. |
| **Inter-pin skew** | CLOSED for outputs, **by simulation**. Measured on the signed-off gate netlist with its own SDF back-annotated, at both corners, 6000 clocks per program (`docs/skew-report.md`, `rtl2/tb/run_skew.py`). Worst skew anywhere **0.707 ns**, at the slow corner: SPI MOSI-SCK, JTAG TDI-TCK and USB D+/D- all 0.707 ns slow / 0.451 ns typ, I2C SDA-SCL 0.681 / 0.402, JTAG TCK-TMS 0.297 / 0.191. Every pair is worse at the slow corner than at the typical one, which is what delay-driven skew must do. Every pair passes with two to three orders of magnitude of margin, because skew does not compare against a setup requirement directly -- it erodes the separation the program creates. The SPI program puts MOSI 15 cycles (~333 ns) ahead of the SCK rise, so setup at the pin is ~332 ns against the 1 ns a W25Q128JV requires (t_DVCH, W25Q128JV-DTR datasheet Revision C, 27 March 2018 per its revision history); I2C is 0.681 ns of skew against t_SU;DAT of 250 ns (UM10204 Rev 7.0). The measurement carries its own non-zero reference -- per-pin clock-to-output of 2660-3367 ps slow and 1636-2087 ps typ -- and `test_skew.py` fails if any pin reads zero, so the Icarus quantization that once reported zero skew everywhere cannot return silently. These figures are for OUTPUT pins only; the input side is the row above and is NOT MEASURED. The I2C figure rests on two transitions per pin, both in the START condition, because no I2C target is attached to release the bus. This entry previously read CLOSED statically, and the static estimate it carried was optimistic by 2.3x on JTAG TCK-TMS and reported the typical corner as worse than the slow one; see docs/writeup.md 4.3 entries 13 and 14. |
...
| item | status |
|---|---|
| **`crc_lfsr16` and `bit_stuffer` have never run on anything physical** | Both are implemented, reachable through the §9 reserved codes, and exercised in simulation -- and neither has run on hardware. The USB work did **not** change that, which is worth stating because it would be easy to assume otherwise: the token uses the 5-bit per-machine CRC, not the wide one, and the receiver destuffs with the `c2` countdown rather than the unit, since USB's rule is a run of ones and a counter is enough (§16.0). The unit's value is in-loop run detection on RECEIVED data, which is CAN's case, and CAN has not been run on hardware either. |
