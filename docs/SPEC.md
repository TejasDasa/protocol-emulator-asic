...
| **Inter-pin skew** | PARTLY CLOSED, and **statically** rather than by simulation. Measured from the signed-off design with OpenSTA (`docs/skew-report.md`): worst-case pin-to-pin skew **5.21 ns** slow / 4.73 ns typ for SPI SCK-MOSI, JTAG TCK-TDI and USB D+/D-, **4.66 ns** for JTAG TCK-TMS, **4.16 ns** for I2C SCL-SDA. Only I2C has a requirement that could be verified -- t_SU;DAT >= 250 ns standard-mode, 100 ns fast-mode (UM10204 Rev 7.0, 1 October 2021) -- and it passes by 245.8 ns. SPI and JTAG setup/hold are per-device and no datasheet value could be extracted on this machine, so **the SPI pair is unjudged and is the one to check against a real target**: 5.21 ns would not clear a part specifying 2-5 ns. Two caveats that matter. The envelope spans different iomux driver selections, so it is an upper bound rather than the skew of any one configuration. And it is static: gate-level simulation annotated SDF successfully and resolved real picoseconds on the clock tree, but the machines never reached a state where they drive pins (`rtl2/sdf/README.md`). |
