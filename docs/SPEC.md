...
| **Slew, capacitance and fanout** | Not clean, post-route on the five-machine D design at the slow corner: **26 max-slew** (3 typ, 0 fast), **7 max-capacitance**, **621 max-fanout**. Routing and setup/hold timing are clean; these are not. Of the fanout violations **320 are inside the CFGMEM macros** — the macro's own `SLICE[*].ROW_AND` and `SELBUF` drive 33 loads against a limit of 8 — and are a property of a vendored macro, not of this design or fixable by resizing it. The remaining 301 fanout, and all the slew and capacitance violations, are in our logic. `RUN_POST_GRT_DESIGN_REPAIR` and `RUN_POST_GRT_RESIZER_TIMING` are both **off**, so nothing repairs slew or fanout against real parasitics; that is the first thing to try and it has not been tried. |
