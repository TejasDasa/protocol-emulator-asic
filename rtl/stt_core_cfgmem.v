// stt_core_cfgmem -- one STT state machine with the row array in tiled
// CFGMEM_IHP16 macros instead of flops. Exists to answer ONE question: do the
// macros place and power under the Tiny Tapeout single-layer PDN?
//
// Identical to stt_core except for the instruction memory. Uses the 21-bit row
// (configuration C); the PDN question is the same for C and D, and C needs no
// decode changes.
//
...
module stt_core_cfgmem #(
  ...
  // ---- instruction memory ---------------------------------------------
  // ---- instruction memory: tiled CFGMEM macros, not flops -------------
  stt_imem_cfgmem #(.ROW_W(ROW_W), .ROWS(ROWS), .ADDR_W(ADDR_W), .NTILE(2))
  u_imem (
      .clk     (clk),
      .rst_n   (rst_n),
      .addr    (row_addr),
